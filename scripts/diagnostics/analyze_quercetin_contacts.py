...
def parse_pdbqt_ligand_pose1(ligand_path):
    """
    Parses only the first pose (MODEL 1) from a Vina output PDBQT file.
    Returns a list of coordinates [X, Y, Z] for all atoms.
    """
    coords = []
...
def main():
    ...
    quercetin_path = os.path.join(project_root, "docking", "vina_output", "MurG_Quercetin_3D_out.pdbqt")
    
    # 1. Parse docked Quercetin pose 1 coords
    lig_coords = parse_pdbqt_ligand_pose1(quercetin_path)
    if not lig_coords:
        print("Could not parse Quercetin pose 1.")
        return
        
    print(f"Parsed {len(lig_coords)} atoms from Quercetin's best docked pose.")
    ...
    print("\n--- Residues in MurG forming contacts (<= 4.5 A) with our Quercetin (Best Pose) ---")
